...
interface ChannelFormProps {
  ...
  /** 渠道是否可加入 Agent 使能列表发生变化 */
  onAgentAvailabilityChange?: (channel: Channel, available: boolean) => void | Promise<void>
  onCancel: () => void
}
...
function isAgentAvailableChannel(channel: Pick<Channel, 'enabled'>): boolean {
  return channel.enabled
}

export function ChannelForm({ channel, onSaved, onAgentAvailabilityChange, onCancel }: ChannelFormProps): React.ReactElement {
  ...
  const lastAgentAvailableRef = React.useRef(channel ? isAgentAvailableChannel(channel) : false)

  React.useEffect(() => {
    lastAgentAvailableRef.current = channel ? isAgentAvailableChannel(channel) : false
  }, [channel])
  ...
  const doAutoSave = React.useCallback(async (
    ...
    try {
      ...
      const available = isAgentAvailableChannel(savedChannel)
      if (available !== lastAgentAvailableRef.current) {
        lastAgentAvailableRef.current = available
        await onAgentAvailabilityChange?.(savedChannel, available)
      }
  ...
  }, [isEdit, channel, onAgentAvailabilityChange])
  ...
  const handleCodexLogin = async (): Promise<void> => {
    ...
    try {
      ...
      if (isEdit) {
        ...
        if (isAgentAvailableChannel(saved)) {
          await onAgentAvailabilityChange?.(saved, true)
        }
  ...
  const doCreate = React.useCallback(async (): Promise<Channel | null> => {
    ...
    try {
      ...
      if (isAgentAvailableChannel(savedChannel)) {
        await onAgentAvailabilityChange?.(savedChannel, true)
      }
  ...
  }, [name, provider, baseUrl, effectiveApiKey, hasRequiredSecret, models, enabled, onAgentAvailabilityChange])
