...
export function registerIpcHandlers(): void {
  ...
  ipcMain.handle(
    ...
    async (_, id: string, title: string): Promise<AgentSessionMeta> => {
      return renameAgentSessionTitle(id, title)
    }
  )
  ...
  // 中止 runtime 自动重试
  ipcMain.handle(
    AGENT_IPC_CHANNELS.ABORT_RUNTIME_RETRY,
    async (_, input: AbortRuntimeRetryInput): Promise<void> => {
      try {
  ...
  // 获取活跃 runtime 状态
  ipcMain.handle(
    AGENT_IPC_CHANNELS.GET_RUNTIME_STATE,
    async (_, input: GetRuntimeStateInput): Promise<AgentRuntimeStateResult> => {
      try {
        return await getAgentRuntimeState(input)
      } catch (error) {
        console.error('[IPC] 获取 runtime 状态失败:', error)
        throw error
      }
    }
  )
