...
| 包 | 版本 | 职责 |
| --- | --- | --- |
| `@proma/electron` | `0.10.21` | Electron 桌面应用 |
| `@proma/shared` | `0.1.23` | 共享类型、IPC 常量、配置和工具 |
| `@proma/core` | `0.2.9` | Provider Adapter、SSE、Shiki 高亮 |
| `@proma/ui` | `0.1.4` | 共享 React UI 组件 |
...
- `agent-orchestrator.ts`：Agent 编排、环境变量、运行时适配器调用、事件流、错误处理。
